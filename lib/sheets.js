...
// Oct-Dec 2025 layout: flat table with Campground/Product/Deal/Date signed/Go Live Date/Rep + two unlabeled cols
function parseFlat(rows) {
  ...
  const headers = raw.map((h) => {
    const t = (h ?? "").trim();
    if (t) return t;
    return EXTRA_LABELS[extraIdx++] || "";
  });
  return trimTrailingEmptyCols(headers, rest).then ? trimTrailingEmptyCols(headers, rest) : trimTrailingEmptyCols(headers, rest);
}
